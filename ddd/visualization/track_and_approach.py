    ...
    try:
        while True:
            ...
            stat += (f" codetect={'Y' if co_detect else 'N'} target={args.target_mm/10:.0f}cm"
                     ...
                     f"{' FINE' if state['fine'] else ''} fps={cam_fps:.0f}")
            cv2.putText(canvas, stat, (8, CAM_H - int(10 * sui)), cv2.FONT_HERSHEY_SIMPLEX,
                        0.4 * sui, (0, 255, 255), 1)
            if near_a is not None:   # 진단줄: 최근접점 raw각도/베어링/거리 (정면 보정용)
                diag = f"near raw={near_a} bearing={lidar_bearing(near_a):+.0f}deg {near_d/10:.0f}cm  (object dead-front -> set FORWARD_ANGLE_DEG=raw)"
